...
const Profil = ({ uid }) => {
    ...
    const callApiProfil = () => {
      ...
      if (profilmenu !== "addedGames") {
        ...
        if (fav === true) {
          ...
          contentprofil.current.classList.remove(
            "page-content-profile-no-data"
          );
      ...
      morecontent.go === true
        ? setFilteredFav(fav?.slice(0, fav.length))
        : setFilteredFav(fav?.slice(0, 5));
      return fav;
    };
    callApiProfil();
  ...
  useEffect(() => {
    const callCommentOfGamesFav = async () => {
      if (user._id != null) {
        const res = await axios.get(
          `${process.env.REACT_APP_API_URL}api/user/allcomments/${user._id}`
        );
  ...
  return (
    ...
    <div
      className={
        currentuser?.status === "admin" ? "sous-menu" : "sous-menu-user"
      }
    >
    ...
    <div className="container">
      ...
      {
        <>
          <div className="up-down-game-profil">
            <span
              onClick={() =>
                setMoreContent({ go: true, elements: filteredfavlength })
              }
            >
              <FontAwesomeIcon icon={faArrowDown} />
            </span>
            <span
              onClick={() => setMoreContent({ go: false, elements: 5 })}
            >
            ...
            <>
              <div className="currentuser-comment">
                <div className="user-info-comment-profil">
                  <h4>
                    <a href={"/profil/"} style={{ fontSize: "20px" }}>
                      {currentuser?.pseudo}
                    </a>{" "}
                    a commenté {currentusergamecomment.titlegame}{" "}
                  </h4>
                  <p className="">
                    {" "}
                    {dateParser(currentusergamecomment.createdAt)}
                    <p className="text-comment-quickview">
                      {currentusergamecomment.text.slice(0, 30)}...
                    </p>
                  </p>
                </div>
                <div className="arrow-comments">
                  <p>
                    <a
                      href={
                        "/game-detail/" + currentusergamecomment.gameId
                      }
                    >
                ...
                <img
                  src={".." + currentusergamecomment.gamepicture}
                  alt="comment of user"
                  style={{ width: "10rem" }}
                />
              </div>
