...
const ModalReview = ({
  ...
  const handleAdd = async (e) => {
    ...
    if (description || files) {
      ...
      await axios
        .post(`${process.env.REACT_APP_API_URL}api/user/review`, formData, {
          withCredentials: true,
        })
        .then((res) => {
          console.log(res);
  ...
  return (
    ...
    >
      <div>
        ...
        <form
          onSubmit={action === "edit" ? handleAddEdit : handleAdd}
          id="sign-up"
        >
          ...
          <div className="footer-modal">
            <div className="upload-modal">
              ...
              <select
                name="gameId"
                onChange={(e) => setCurrentGameSelect(e.target.value)}
              >
                <option disabled selected>
                  selectionne un jeu
                </option>
                {filteredGames?.map((game, index) => {
                  return (
                    <>
                      {" "}
                      <option value={game._id} key={index}>
                        {game.title}
                      </option>
                    </>
                  );
              ...
              <select onChange={(e) => setPlateform(e.target.value)}>
                <option name="plateform" disabled selected>
                  selectionne un plateform
                </option>
                {allplateform?.map((plateforme, index) => {
                  return (
                    <>
                      {" "}
                      <option value={plateforme} key={index}>
                        {plateforme}
                      </option>
                    </>
                  );
