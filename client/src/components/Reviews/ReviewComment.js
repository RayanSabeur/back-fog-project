...
import EditDeleteCommenter from '../Details/EditDeleteCommenter';
...
import { faBook, faBookmark, faCircleXmark, faGamepad, faPen, faTrash, faXmark,faPlayCircle, faMessage } from '@fortawesome/free-solid-svg-icons';
...
const ReviewComment = ({comment}) => {
    ...
    const handleDeleteComment = (commentid) => {
     ...
     const handleDelete = async (game) => {
       ...
       await axios.delete(
          `${process.env.REACT_APP_API_URL}api/user/review/${game}`,
          {
              withCredentials: true
          }
          ).then((res) => {
   ...
   <>   
    ...
    <p style={edit == true && comment.id == currentmsg ? {display: 'none', color: '#9DAED2',fontSize: '20px'} : {color: 'black', color: '#9DAED2',fontSize: '20px', wordBreak: 'break-all'}}>{comment.text}</p>
      ...
      <EditDeleteCommenter comment={comment}  edit={edit} setEdit={setEdit} currentmsg={currentmsg} action={'editcommentreview'} currentelm={comment.id}/>
      ...
      <div className="comment-icon">
            ...
            {showComments && <ReviewCommentPost post={comment}  edit={edit} setEdit={setEdit}/> }
       <hr/>
